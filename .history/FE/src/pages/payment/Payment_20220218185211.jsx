...
const Payment = () => {
    ...
    const paymentHandle = () =>{
        ...
        payment({
            action: "payment",
            email: user.email,
            item: items.tiem + params.id+',',
          //  total:Number(total)+Number(item.price.split(' ')[0].replaceAll('.',''))
          });
    }
...
CHƯƠNG TRÌNH <span>THẺ XANH CHÀO NGÀY MỚI</span> <a href="#">Xem chương trình chi tiết</a></p>
      ...
      <div className="payment__card">
            ...
            <div className="payment__btn">
                <button type='submit' className={`btn btn__payment`} onClick={paymentHandle}>Thanh toán</button>
            </div>
      </div>
  </form>;
};
