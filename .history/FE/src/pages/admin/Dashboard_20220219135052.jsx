...
import { getTran } from "../../reduxToolkit/apiRequest";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    getTransaction({ action: "fetchTrans" }).then((res) => setTrans(res));
  }, []);
console.log(trasn)
  return (
    <DashboardWrapper>
  ...
  return (
    <>
      <td className="widgetUser">
        <Avatar size="xl" src={props.img} bordered zoomed squared color={props.status=='active' ? "success" : props.status == 'pending' ? "warning" : 'error'} />
...
const ListUserTransactions = (props) => {
  return (
