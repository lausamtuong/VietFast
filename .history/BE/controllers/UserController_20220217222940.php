<?php
   require_once('model/DbModel.php');
    ...
    public function setPayment($data, $email){
        $con=$this->connect();
        $data = $_POST["data"];
        $email = $_POST["email"];
        $toDatabse = base64_encode(serialize($data)); 
        // $sql = "UPDATE `account` SET `item` = '$toDatabse' WHERE `email`='$email'";
        $sql ="SELECT  `item` FROM `account` WHERE `email`='$email'";
        $retval = mysqli_query( $con,$sql );
        $fromDatabase = unserialize(base64_decode($con,$retval)); //Getting Save Format 
        // $fromDatabase = unserialize(base64_decode($data))
       var_dump(unserialize($fromDatabase));
